...
on fuel cells, accumulators and FV panels for small powers\pagebreak{}}
...
which we can follow through the entire work. Lets say, we want to
make an energy independent unit - ecologic house, for example.

Lets add renewable energy source (fotovoltaic panels) and a system
...
\begin{comment}
In this work we want not only solving this problem, but also to make
this text usefull for future coworkers,
\end{comment}
...
formulation. Next we will state arising interesting questions, which
we will try to answer - to complement other works.
...
Lets summarize other works and interesting ideas from the relevant
fields.
...
\cite{EnergyAccumulation} and creates a possibility for research
of various systems and using mathematical tools to model and optimize
theese systems. One example of such a system are the standalone energetic
units \cite{MassimoStandalone}.
...
for example \cite{IpsakisPowerManagmentStrategies}), and some work
is also focused on specific installations \cite{Abdelkafi20141},
\cite{Sarioglu20147}, \cite{Taher2014268}, \cite{Cai2014212}.

Application of optimization in the theory of optimal control is described
in the books \cite{EvansControlCourse}, \cite{Lewis2012optimal},
\cite{Warga1972optimal}. An example of a method used for global optimization
is an application of metropolis algorithm, so called Simulated Annealing
\cite{SimulatedAnnealing} (that is used too for the design of electric
circuits or in chemistry \cite{Chaudhuri1997733}). There is even
a method taking in account the uncertainity of data, Stochastic Annealing
\cite{PaintonStochastic}. This method is used in \cite{GiannakoudisOptimumDesign}
for optimization of the parameters of the control strategy.

Another interesting principle, the Pontryagin principle, is used to
prolong the lifetime of such systems \cite{XuPontryaginStrategy,ProlongingLifetime,ZhengPontryaginProlonging},
or optimization problems are solved for vehicles \cite{Maalej2014668}.
...
Needed to say, that often, new algorithms of global optimization are
...
this and another possibilities of algorithms is the text \cite{ExperimentalAlgorithms}). 
...
in case we need to evaluate the model more times (as in our case).


\subsection*{Photovoltaics}

When looking at photovoltaic (PV) panels, if we want a better model
than saying ``general power source\char`\"{}, we need to consider
...
present. Thats the reason, why the following equivalent circuit ``5
parameter circuit\char`\"{} is used (there exists even the so called
``7 parameter circuit\char`\"{} which has another diode in parallel
to the first diode). 

\includegraphics{275px-Solar_cell_equivalent_circuit\lyxdot svg}
...
\item $C_{i}$ constants
\item $I_{L}$light current\nomenclature{current}{light current} % \nomenclature{$I_L$}{light current\refeqpage}% 
...
how to compute the parameters from manufacturers supplied information
from datasheet and notes on behavior with changing temperature and
finally even a fortran code for PV panels. Also, the efficiency of
PV energy generation is said to be from 5 to 20\%. 
...
in \cite{beyer2004robust,salas2006review}, even using neural networks
...
case, where electrolyzer works better at lower loads, battery can
...
\includegraphics[scale=0.2]{EqivLeadAcid}
...
The indexes ``ch\char`\"{} stand for charging and ``dch\char`\"{}
...
for $SOC$ we have a model with efficieency ($\eta_{ac}$), discharge
rate ($\sigma_{ac}$) and charge or discharge current $I_{ac}$.
...
$SOC$ cant be directly measured, it needs to be derived from the
...
Electrolyzer and fuel cell both basically operate from simple chemical
...
By adding electrical power, the reaction occurs from left side to
...
to fuel ratio). The higher heating value is $285.83\,kj\,mol^{-1}$,
the pure standart heat formation of the product water. The lower heating
value is $120\,MJ\,kg^{-1}$, calculated from the fact, that the product
oxygen goes out unused as a steam (at said $150$ \textdegree C ).
...
alkaline solution) electrolyzers and proton exchange membrane electrolyzer.
...
\[
n_{H_{2}}=\frac{N_{cell}I_{el}}{n_{e}F}\eta_{F}
\]
...
to be determined by experimental measure). 
...
And the calculation for the amount of hydrogen produced is (from \cite{olubukunmi2011solar}),
where $\epsilon$ is the fuel cell efficiency:

\[
n_{H_{2}}=\frac{1}{\epsilon}\cdot0,01866\frac{mol}{hr\cdot Amp},\,m_{H_{2}}=\frac{1}{\epsilon}\cdot3,77\cdot10^{-5}\frac{kg}{hr\cdot Amp}
\]


The simplest way to model fuel cell V-I relationship is to look at
it as a reverse electrolyzer, this course is followed by the following
...
effects of mass. The model computes the cell voltage $V_{cell}$ using
open circuit voltage $E$, current density $i$, model parameter $b$,
current at the inflection point of the V-I curve $I_{d}$ and slope
$\Delta$ at the linear stage of ohmic overvoltage:

\[
V_{cell}=E+\frac{b}{\ln(1/I_{d}\,i)}-(\Delta-\frac{b}{4I_{d}})\,i
\]


The parameters ($K\in\{E,\,I_{d},\,b,\,\Delta\}$) vary with fuel
cell temperature $T$ and oxygen partial pressue $p_{O_{2}}$:
...
powers. But can be used even for electrolyzer ($p_{O_{2}}$ is then
the operating pressue for the electrolyzer at which the oxygen is
produced).
...
Where $V_{rev,FC}$ is the reversible voltage, $i$ the current density
(fuel cell current per unit area of the electrode, in units of milliamps
per square centimeter),$i_{0}$ and $B$ the Tafel parameter ($i_{0}$is
the value on the Tafel plot when the current begins to move away from
zero), $a_{T}$ the Tafel slope, $r$ the resistance (in $\Omega\cdot cm^{2}$),
finally the model parameters $m$, $l$ representing the overvoltage
due to mass transport limits. All theese parameters can depend on
temperature.

Hydrogen and oxygen then flows according to equations (by Faradays
law):

\[
H_{2,g}=\frac{nI}{2F\eta_{F}},\,O_{2,g}=\frac{nI}{4F\eta_{F}}
\]


All theese models are empirical and are important only because of
their low computational complexity. For greater complexity, there
are of course more sophisticated models using partial differential
equations. There is also a nice book about fuel cells, that examines
every aspect of fuel cell design and explains the details of how fuel
cells works \cite{larminie2003fuel}.
...
need to add the compressor into the equation (like it is done in ).
...
specific problems settings. Optimal control is applied optimization
together with variational calculus \cite{milyutin1999calculus} and
has similar subproblems.
...
function'' or ``cost function\char`\"{} $J$ that consists of final
time objective and integrated Lagrangian) of a dynamic system, evolving
in time, controlled by a function (system dynamics).
...
Optimization problems have many subclasses (as the picture shows)
and optimal control is not an exception.

\includegraphics[width=0.5\textwidth]{treeoptimiz}
...
later, one gives the name ``adjoint variable\char`\"{} to one of
the derivatives, so it is conveniet to use the name). It is essential
...
Dynamic programming needs discretized problem formulation, but going
to the limit gives rise to the HJB equations.
...
problem is the, so called, curse of dimensionality - when state and/or
...
\item Collocation - disretized controls and states
\end{itemize}
\end{itemize}
...
meaning, that the Jacobians have many zero elements). Collocation
can treat better even unstable systems, but refining the grid results
in new problem with higher dimensions.

Multiple shooting discretizes the control, as single shooting, and
additionally the time scale. So it is multiple shooting, because it
is like running single shooting algorithm again from the starting
...
values on the next interval). Multiple shooting is only block sparse,
but is easy to paralelize and can treat even unstable systems well.

Numerous implementations of numerical optimal control are availible,
namely \cite{Houska2011a}.
...
of the said strategy). So lets ask the question:
\begin{itemize}
...
system parameters the optimal control strategy is still the same?
\end{itemize}
...
$u_{2}$, that will be in ampers (see next).

Because we have two state variables - the state of charge of the accumulator
$B(t)$ and the stored hydrogen $S(t)$, our state is two dimensional
real valued function $x(t)=(x_{1}(t),x_{2}(t))=(B(t),S(t))$. Theese
quantities are bounded by $B(t)\leq B_{max}$ and $S(t)\leq S_{max}$,
bounds to maximal battery charge and maximal hydrogen stored (in means
of power we used to create the hydrogen). 

The boundary condition is $P_{src}(t)=P_{in}(t)-P_{demand}(t)$, difference
between input power and power demand.

The control function $u$ is a two dimensional function of time and
will define the power flow (so its units will be Watts):
\begin{itemize}
...
\end{itemize}
\item The second component tells us about the flow of (energy stored in)
hydrogen:
...
Again, here we can model the efficiency ($P_{H_{2}}(u_{2})=u_{2}\eta_{H_{2}}$)
and even nonlinear characteristics of the efficiency. Definitely the
term will be different for $u_{2}$ positive and negative, because
this equation models two different devices:
\begin{itemize}
\item $u_{2}>0$ ... The electrolyzer is turned on, using the current$u_{2}$.
There is a bound, how much current can flow $u_{2}\in[P_{H_{2}+min},P_{H_{2}+max}]$
\item $u_{2}=0$ ... No change in hydrogen supply
\item $u_{2}<0$ ... The fuel cell is turned on, giving us the current $-u_{2}$.
There is a bound, how much current can we get $-u_{2}\in[P_{H_{2}-min},P_{H_{2}-max}]$
\end{itemize}

This design follows the wise rule, that the electrolyzer and fuel
cell should not operate simultaneously (because that would be a waste
of power). The bounds are, for example, the technical specifications
set by manufacturer. Why do we use current for $u_{2}$ and not power?
Because the equations will give us how the voltage depends on current
(and from that we can get the power dependency on current).
...
to grid wouldnt give us such an amount of money as we would lose by
buying the power.

Genrally we can think of any $pcost$ function, in the solution and
scrupts below we will simply use $pcost(x)=c_{1}x\mbox{ if }x\geq0\mbox{, otherwise }=c_{2}x$,
for $c_{1},c_{2}\geq0$.
...
Now we will look at different formulations and use methods like in
...
By the way, because our state equations does not depend on time, our
...
allowing countable number of disconiuities would araise the need of
difficult rewriting the problem, as in \cite{gelfand1964calculus}).
...
are approaching the limit if given functional is continuous.
...
-----------\textgreater{}-Numericky kody brat z http://plato.la.asu.edu/bench.html\textless{}\textless{}---------------------------
...
following question ``What is the optimal control function (or the
most economic setting) for photovoltaic and hydrogen system and what
is the power managment strategy of such system?\char`\"{}. Lets suppose,
...
and sufficient). The source codes are in the directory ``fit\char`\"{}.

\includegraphics{fittedelectrolyzerplot}\includegraphics{fittedfuelcell}

Results of the fitting for electrolyzer (left) and fuelcell (right),
x axis is current density ($mA\cdot m^{-2}$), y axis is voltage ($V$).

Actual values of parameters are kept in the file consts.m together
with all the other model constants and numerical settings.
...
this problem. In the main directory, there is implementation of multiple
shooting using fmincon.
...
to number of discretization points beggining at $2^{\mbox{cLogBeg}}$
and ending at $2^{\mbox{cLogEnd}}$ (constants from \texttt{consts.m}),
copying the results from previous run to the next run, because in
the optimization algorithms, the choice of starting point in the search
space matters. Because of that, some iterations can fail to satisfy
the bounds thinking that the problem is infeasible - that is automatically
fixed when the number of discretization points grows bigger. Because
then there will be more decision points, that can steer the system
